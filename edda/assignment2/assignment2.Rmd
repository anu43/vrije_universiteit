...
myt=mystat(lm(nausea~treatstar))
myt
hist(tstar)
```
...
pvalue
```
The obtained p-value is very high thus we conclude that the variables are not dependent and we accept the null hypothesis ($H_0$).
...
In the following plot, we clearly see an influence point: the Cook’s distance is `r round(cooks.distance(model),2)[5]` for the leverage point at index 5 and `r round(cooks.distance(model),2)[8]` for the leverage point at index 8.
```{r}
...
Influence PLot
```{r}
influencePlot(model, main="Influence Plot", sub="Circle size represents Cook's Distance")
```
